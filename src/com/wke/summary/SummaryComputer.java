...
public class SummaryComputer {
	...
	public SummaryComputer(int len, boolean isSplitSummary, String title,
			String content) {
		this.len = len;
		this.title = title;
		this.content = content;
	...
	public Summary toSummary(String query) {
		...
		for (Term term : parse) {
			...
			keywords.add(new Keyword(term.getName(),
					term.termNatures().allFreq, 1));
		}
	...
	private void computeScore(Sentence sentence, SmartForest<Double> forest) {
		SmartGetWord<Double> sgw = new SmartGetWord<Double>(forest,
				sentence.value);
